...
### What this picks up from the native SDKs
...
- **Added:** `getFeatureEntitlement(featureKey)` and `getPermissionEntitlement(permissionKey)` — query whether the current user is entitled to a specific feature/permission from Dart, returning an `Entitlement { isEntitled, justification }`. Previously only `loadEntitlements()` (a load trigger) was exposed, so apps had to decode JWT claims by hand. Bridged to the native `getFeatureEntitlements`/`getPermissionEntitlements` on both platforms (evaluated on-device from the state populated by `loadEntitlements`). New Dart `Entitlement` model + barrel export; Android + iOS method-call handlers; unit tests (delegation + method-channel parsing); `docs/advanced.md` examples. Closes a wrapper parity gap vs. React Native / native SDKs.
- **CI:** added an ungated `dart-tests` job (`flutter analyze lib test` + `flutter test`) so unit tests run on PR branches (the existing `flutter test` job was skipped on PRs).

All version references bumped: plugin + example app Gradle pins, SPM manifests, and docs.
